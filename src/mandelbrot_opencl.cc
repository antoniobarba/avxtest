...
void mandelbrot_opencl(easycl::Kernel& kernel, easycl::Device& device, void *points, int w, int h)
{
    ...
    kernel.run(device.getQueue(), std::vector<size_t>{(uint32_t)w, (uint32_t)h}, clBuffer, w, h);
